<div
	class="relative bg-cover bg-center py-32 text-white md:py-48"
	style="background-image: url('/photos/hair-styling.jpg');"
>
	<!-- Cinematic gradient overlay: dark at bottom, lighter at top -->
	<div
		class="absolute inset-0"
		style="background: linear-gradient(to top, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.45) 50%, rgba(0,0,0,0.25) 100%);"
	></div>

	<div class="relative z-10 container mx-auto px-4 text-center">
		<!-- Decorative gold line above tagline -->
		<div
			class="mx-auto mb-6 h-px w-16"
			style="background: linear-gradient(90deg, transparent, var(--color-gold), transparent);"
		></div>

		<p class="mb-4 text-sm tracking-widest text-[var(--color-gold)] uppercase">
			Expert Hair & Beauty
		</p>

		<h1
			class="mb-6 font-serif text-4xl font-bold tracking-tight md:text-5xl lg:text-6xl"
			style="text-shadow: 0 2px 20px rgba(0,0,0,0.4);"
		>
			Welcome to <span class="text-[var(--color-gold)]">Studio210</span>
		</h1>

		<p class="mx-auto mb-10 max-w-3xl text-xl md:text-2xl">
			Expert hair styling, colouring, and treatments in the heart of Bestwood Park, Nottingham
		</p>

		<div class="flex flex-col justify-center gap-4 sm:flex-row">
			<a
				href="/services"
				class="inline-block rounded-full bg-[var(--color-gold)] px-8 py-3 text-lg font-semibold text-[var(--color-navy)] shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-[var(--color-gold-light)] hover:shadow-xl active:scale-[0.98]"
			>
				View Services
			</a>
			<a
				href="/booking"
				class="inline-block rounded-full bg-white px-8 py-3 text-lg font-semibold text-[var(--color-navy)] shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-100 hover:shadow-xl active:scale-[0.98]"
			>
				Book Now
			</a>
		</div>
	</div>
</div>
